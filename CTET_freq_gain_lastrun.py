...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/yiranli/Downloads/UMich/Research/Thesis/CTET_freq_gain/CTET_freq_gain_lastrun.py',
            ...
            while continueRoutine:
                ...
                if key_resp_easy.status == STARTED and not waitOnFlip:
                    ...
                    if len(_key_resp_easy_allKeys):
                        key_resp_easy.keys = _key_resp_easy_allKeys[-1].name  # just the last key pressed
                        key_resp_easy.rt = _key_resp_easy_allKeys[-1].rt
                        # was this correct?
...
for thisAftereasy_loop in aftereasy_loop:
            ...
            while continueRoutine:
                ...
                if key_resp_normal.status == STARTED and not waitOnFlip:
                    ...
                    if len(_key_resp_normal_allKeys):
                        key_resp_normal.keys = _key_resp_normal_allKeys[-1].name  # just the last key pressed
                        key_resp_normal.rt = _key_resp_normal_allKeys[-1].rt
                        # was this correct?
    ...
    for thisAfternormal_loop in afternormal_loop:
        ...
        for thisOuter in outer:
            ...
            text_feedback.text = "Congratulations! Because of the correct responses you made during this trial, you WON " + str(totalgain) + " dollar."
            text_original.text = "$" + str(bonus)
            text_current.text = "$" + str(bonus + totalgain)
            text_loss.text = "$" + str(totalgain)
            
            thisExp.addData('oldtotal', bonus)
            thisExp.addData('totalgain', totalgain)
            thisExp.addData('newtotal', bonus+totalgain)
            # keep track of which components have finished
